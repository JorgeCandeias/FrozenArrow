...
public sealed class OutputFormatter
{
    ...
    private string FormatTable(List<ProfilingResult> results)
    {
        ...
        sb.AppendLine(string.Format("  {0,-30} {1,12} {2,10} {3,10} {4,10} {5,10} {6,12}",
            "Scenario", "Median (µs)", "Min", "Max", "StdDev", "M rows/s", "Alloc"));
        ...
        foreach (var r in results)
        {
            ...
            var stabilityIndicator = r.IsStable ? "" : " ?";
            sb.AppendLine(string.Format("  {0,-30} {1,12:F1} {2,10:F1} {3,10:F1} {4,10:F1} {5,10:F2} {6,12}{7}",
                r.ScenarioName, r.MedianMicroseconds, r.MinMicroseconds, r.MaxMicroseconds, r.StdDevMicroseconds, r.MillionRowsPerSecond, alloc, stabilityIndicator));
        }

        sb.AppendLine();
        
        // Show outlier removal and stability info
        var outlierResults = results.Where(r => r.OutliersRemoved > 0).ToList();
        if (outlierResults.Count > 0 && _config.RemoveOutliers)
        {
            sb.AppendLine("  Outliers removed:");
            foreach (var r in outlierResults)
            {
                sb.AppendLine($"    {r.ScenarioName}: {r.OutliersRemoved} outlier(s) removed");
            }
            sb.AppendLine();
        }
        
        // Show stability warnings
        var unstableResults = results.Where(r => !r.IsStable && r.StabilityWarning != null).ToList();
        if (unstableResults.Count > 0)
        {
            sb.AppendLine("  ? STABILITY WARNINGS:");
            foreach (var r in unstableResults)
            {
                sb.AppendLine($"    {r.ScenarioName}: {r.StabilityWarning}");
            }
            sb.AppendLine();
        }
        ...
        if (_config.Verbose)
        {
            foreach (var r in results.Where(r => r.PhaseDetails is not null))
            {
                ...
                {
                    sb.AppendLine($"    {name,-25} {phase.AverageMicroseconds,10:F1} µs ({phase.PercentageOfTotal,5:F1}%)");
                }
                sb.AppendLine();
            }
        }
